...
export class ComposioProvider implements MCPProvider {
        ...
        try {
            ...
            for (const [service, authConfigId] of Object.entries(this.AUTH_CONFIGS)) {
                try {
                    ...
                    const activeConnections = connections?.items?.filter((conn: ComposioConnection) =>
    ...
    async getAuthUrl(
        ...
        try {
            ...
            const activeConnections = existingConnections?.items?.filter((conn: ComposioConnection) =>
                conn.status === 'ACTIVE' && conn.authConfig?.id === authConfigId
            ) || [];
    ...
    async verifyAuth(authId: string): Promise<boolean> {
        ...
        try {
            ...
            return connections?.items?.some((conn: ComposioConnection) => conn.status === 'ACTIVE') || false;
        } catch (error) {
        ...
        try {
            ...
            for (const [service, authConfigId] of Object.entries(this.AUTH_CONFIGS)) {
                try {
                    ...
                    allConnections.push(...activeConnections.map((conn: ComposioConnection) => ({
                        id: conn.id,
                        userId,
                        providerId: this.name,
                        providerType: this.type,
                        status: (conn.status === 'ACTIVE' ? 'connected' : 'disconnected') as 'connected' | 'disconnected' | 'error',
                        metadata: {
                            ...conn,
                            service,
                        },
    ...
    async revokeConnection(connectionId: string, userId: string): Promise<void> {
        ...
        try {
            ...
            const connection = connections?.items?.find((conn: ComposioConnection) => conn.id === connectionId);
            if (connection) {
    ...
    private convertComposioToolToMCPTool(composioTool: ComposioTool, service: string, userId?: string): MCPTool {
